package com.cwuom.chiralcaptcha.chiral.carbon;

import static com.cwuom.chiralcaptcha.util.Utils.performVibrateClick;
...
import com.cwuom.chiralcaptcha.activity.ChiralCaptchaActivity;
import com.cwuom.chiralcaptcha.chiral.carbon.util.IndexFrom;
...
public class MoleculeView extends View {
    ...
    public boolean onTouchEvent(MotionEvent event) {
        if (event.getAction() == MotionEvent.ACTION_UP) {
            ...
            Logger.d("MoleculeView", "Touch Position: (" + wx + ", " + wy + ")");
    ...
    public void switchChiral(@IndexFrom(1) int N) {
    ...
    private void calcScaleFactor(int width, int height) {
        ...
        width -= (int) (getPaddingLeft() + getPaddingRight() + fontSize * 2);
        height -= (int) (getPaddingTop() + getPaddingBottom() + fontSize * 2);
        if (rx * ry == 0) {
            if (rx == ry) {
                scaleFactor = 1;
            } else if (rx != 0) {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (molecule != null && scale > 0) {
            ...
            if (avl > 0) {
                float ref = dip2px(getContext(), 40);
                if (avl * scale > ref) {
                    scale = ref / avl;
                }
            }
    ...
    public void displayAnswer(HashSet<Integer> answer) {
        unselectedAllChiral();
    ...
    public static void initTextColor(){
        boolean isNightMode = AppConfig.isDarkTheme(Objects.requireNonNull(ChiralCaptchaActivity.getAppContext()));
        if (isNightMode) {
